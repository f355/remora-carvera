...
Comms::Comms() : spiSlave(MOSI1, MISO1, SCK1, SSEL1) {
...
void Comms::init() {
  ...
  spiDMAtx1->channelNum(MODDMA::Channel_0)
  ...
  spiDMAtx2->channelNum(MODDMA::Channel_1)
      ->srcMemAddr((uint32_t)ptrTxData)
      ->dstMemAddr(0)
      ->transferSize(SPI_BUFF_SIZE)
      ->transferType(MODDMA::m2p)
  ...
  spiDMArx1->channelNum(MODDMA::Channel_2)
      ->srcMemAddr(0)
  ...
  spiDMArx2->channelNum(MODDMA::Channel_3)
      ->srcMemAddr(0)
...
bool Comms::getStatus(void) { return this->SPIdata; }

void Comms::setStatus(bool status) { this->SPIdata = status; }

bool Comms::getError(void) { return this->SPIdataError; }

void Comms::setError(bool error) { this->SPIdataError = error; }