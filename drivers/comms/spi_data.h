...
typedef struct {
  uint16_t header;  // should be set to SPI_DATA_HEADER by LinuxCNC, the machine should check it
  ...
  uint16_t footer;       // should be set to SPI_DATA_HEADER by LinuxCNC, the machine should
                         // check it to make sure the struct size is correct
} linuxCncData_t;

// Data originating on the machine side
typedef struct {
  uint16_t header;                 // should be set to SPI_DATA_HEADER by the machine, LinuxCNC should check it
  ...
  uint16_t footer;                 // should be set to SPI_DATA_HEADER by the machine, LinuxCNC should
                                   // check it to make sure the struct size is correct
} machineData_t;
