...
#define SPI_DATA_HEADER 0x1337DA7A
#define SPI_DATA_FOOTER 0xDA7ADEAD
...
typedef struct {
  uint32_t header;  // should be set to SPI_DATA_HEADER by LinuxCNC, the machine should check it
  ...
  uint16_t outputs;                             // output pin states (bitmask)
  // TODO do we need this, or joint_freq_command=0 is enough?
  uint8_t joint_enable;  // enable/disable stepgens (bitmask)
  uint32_t footer;       // should be set to SPI_DATA_HEADER by LinuxCNC, the machine should
                         // check it to make sure the struct size is correct
} linuxCncData_t;

// Data originating on the machine side
typedef struct {
  uint32_t header;                 // should be set to SPI_DATA_HEADER by the machine, LinuxCNC should check it
  ...
  uint32_t footer;                 // should be set to SPI_DATA_HEADER by the machine, LinuxCNC should
                                   // check it to make sure the struct size is correct
} machineData_t;
